            ...
            <div v-if="activeShare.kind !== 'webpage'" class="share-footer">
              <div class="share-meta">
                <div v-if="activeShare.kind === 'video' || activeShare.kind === 'livestream'" class="video-control-panel">
                  ...
                  <input
                    v-if="activeShare.kind !== 'livestream'"
                  ...
                  <template v-else>
                    <input
                      class="progress-slider livestream-progress"
                      type="range"
                      min="0"
                      :max="Math.max(livestreamSeekableDuration, 0.1)"
                      :step="0.1"
                      :value="Math.max(0, Math.min(livestreamSeekableOffset, livestreamSeekableDuration))"
                      :disabled="!canLocalControlSharedVideo"
                      @input="handleLivestreamSeek"
                      @change="handleLivestreamSeek"
                    />
                    <span class="live-badge" :class="{ paused: !sharedVideoUi.playing }">LIVE</span>
                  </template>
                  <span class="time-label">
                    <template v-if="activeShare.kind === 'livestream'">
                      {{ formatDuration(sharedVideoUi.currentTime) }}
                    </template>
                    <template v-else>
    ...
    <div v-if="showLivestreamDialog" class="modal-overlay" @click.self="closeLivestreamDialog">
      <div class="modal-content webpage-dialog">
        ...
        <div class="modal-body">
          ...
          <label class="input-label" for="livestream-url-input" style="margin-top:12px;">输入直播地址</label>
          <input
          ...
          />
          <p class="input-hint">{{ livestreamHint }}</p>
        </div>
        <div class="modal-footer">
...
const livestreamSeekableDuration = computed(() => {
  if (activeShare.value?.kind !== 'livestream' || !sharedVideoRef.value) return 0
  ...
  // fallback: 用最近60秒作为回看窗口
  return 60
})
const livestreamSeekableOffset = computed(() => {
  if (activeShare.value?.kind !== 'livestream' || !sharedVideoRef.value) return 0
  const video = sharedVideoRef.value
  if (video.seekable && video.seekable.length > 0) {
    const start = video.seekable.start(0)
...
const livestreamPlaceholder = computed(() => {
...
function restartSharedVideoUiTicker() {
  ...
  if (!shouldUseSyncedVideoUi() && activeShare.value?.kind !== 'livestream') {
    return
  }
  if (!activeShare.value?.sync?.playing && activeShare.value?.kind !== 'livestream') {
    return
  }
  if (!canGlobalControlShare.value && sharedVideoLocalPaused.value) {
    return
...
function syncSharedVideoElementSource() {
  ...
  if (activeShare.value.kind === 'livestream' && activeShare.value.url) {
    // 如果已经在播放同一个直播流，不要重新初始化
    if (currentLivestreamUrl.value === activeShare.value.url && (hlsInstance || flvPlayerInstance)) {
      return
    }
    if (video.srcObject) {
      video.srcObject = null
  ...
  try {
    ...
    await livekitSubscriberSession.connectSubscriber({
      ...
      onTrack: (track) => {
        ...
        // 非管理员本地暂停时不 attach 新 track，保持暂停时的画面
        if (!canGlobalControlShare.value && sharedVideoLocalPaused.value) {
          return
        }
...
function initLivestreamPlayer(url) {
  ...
  const protocol = activeShare.value?.livestreamProtocol || detectStreamProtocol(url)
...
function confirmLivestreamShare() {
  ...
  // HLS/FLV 直接播放
  startLivestreamShare(url, fileName, detectedPlatform)
}
...
async function resolveAndStartLivestream(roomUrl, platform) {
  try {
    ...
    // 使用服务器返回的协议类型（优先hls）
    const protocol = data.protocol || detectStreamProtocol(data.streamUrl) || 'hls'
    startLivestreamShare(data.streamUrl, fileName, protocol)
  } catch (err) {
...
function startLivestreamShare(streamUrl, fileName, protocol) {
  closeSharedMedia()

  const shareId = 'livestream-' + Date.now()
  const streamProtocol = protocol || detectStreamProtocol(streamUrl) || 'hls'
  ...
  pushSystemMessage('开始共享直播: ' + fileName)

  nextTick(() => {
    initLivestreamPlayer(streamUrl)
  })
}
...
function handleSharedVideoLoaded() {
  ...
  if (activeShare.value?.kind === 'video' && activeShare.value.sync) {
    applyVideoSync(activeShare.value.sync, true)
  } else if (activeShare.value?.kind === 'livestream' && activeShare.value.sync) {
    // 直播流只同步播放/暂停状态，不同步时间位置
...
function handleSharedVideoTimeUpdate() {
  ...
  if (sharedVideoRef.value && !shouldUseSyncedVideoUi()) {
    ...
    if (activeShare.value?.kind === 'livestream') {
      ...
      sharedVideoUi.duration = Math.max(sharedVideoUi.currentTime, Number(activeShare.value?.sync?.duration || 0))
    } else {
      sharedVideoUi.duration = rawDuration
    }
    sharedVideoUi.playing = Boolean(!sharedVideoRef.value.paused && !sharedVideoRef.value.ended)
  }
  ...
  emitShareControl('heartbeat', {
    ...
    playing: true,
    duration: activeShare.value?.kind === 'livestream' ? 0 : sharedVideoUi.duration
  })
}
...
function toggleSharedVideoPlayback() {
  ...
  if (canGlobalControlShare.value) {
    ...
    if (sharedVideoRef.value.paused) {
      ...
      if (sharedVideoRef.value.ended && activeShare.value?.kind !== 'livestream') {
        try {
      ...
      // 直播流恢复播放时跳到最新位置
      if (activeShare.value?.kind === 'livestream' && sharedVideoRef.value.seekable && sharedVideoRef.value.seekable.length > 0) {
        const end = sharedVideoRef.value.seekable.end(sharedVideoRef.value.seekable.length - 1)
        if (Number.isFinite(end)) {
          try {
            sharedVideoRef.value.currentTime = end
          } catch (e) {
            console.error('直播跳转到最新位置失败:', e)
          }
        }
      }
      suppressShareEvents(500)
  ...
  if (sharedVideoRef.value.paused) {
    ...
    if (activeShare.value?.kind === 'livestream') {
      // 直播流恢复播放时跳到最新位置
      if (sharedVideoRef.value.seekable && sharedVideoRef.value.seekable.length > 0) {
        const end = sharedVideoRef.value.seekable.end(sharedVideoRef.value.seekable.length - 1)
        if (Number.isFinite(end)) {
          try {
            sharedVideoRef.value.currentTime = end
          } catch (error) {
            console.error('直播跳转到最新位置失败:', error)
          }
        }
      }
      sharedVideoUi.playing = true
...
function handleLivestreamSeek(event) {
  if (!canLocalControlSharedVideo.value || activeShare.value?.kind !== 'livestream' || !sharedVideoRef.value) {
    return
  }
  const video = sharedVideoRef.value
  let nextTime = Number(event.target.value)
  ...
  // 直播回看：seek 到 seekable 范围内的位置
  if (video.seekable && video.seekable.length > 0) {
    const start = video.seekable.start(0)
    const targetTime = start + nextTime
    const end = video.seekable.end(video.seekable.length - 1)
    try {
      video.currentTime = Math.min(targetTime, end)
    } catch (e) {
      console.error('直播回看 seek 失败:', e)
    }
  }
}
