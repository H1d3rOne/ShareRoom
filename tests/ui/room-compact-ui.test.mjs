...
const roomServer = fs.readFileSync('server/server.js', 'utf8')
...
test('房间页 socket 连接不应强制 websocket 单传输，需保留 polling 回退', () => {
  assert.match(roomVue, /socket\.value = io\(SIGNAL_SERVER_URL, \{/)
  assert.doesNotMatch(roomVue, /socket\.value = io\(SIGNAL_SERVER_URL, \{[\s\S]*transports:\s*\[\s*'websocket'\s*\]/)
})
...
test('共享区权限与视频控制按管理员全局/成员本地分层', () => {
  ...
  assert.match(roomVue, /<button v-if="canOpenGameMenu" class="toolbar-btn game-menu-btn" :class="\{ active: showGameMenu \|\| activeGame \|\| gameInvite \}" @click="toggleGameMenu">/)
  assert.match(roomVue, /function toggleGameMenu\(\) \{[\s\S]*showGameMenu\.value = false[\s\S]*openGameMenu\(\)/)
...
test('屏幕共享启动不再自动探测本地远控 agent，远控功能已移除', () => {
  assert.doesNotMatch(roomVue, /async function startScreenShare\(\) \{[\s\S]*checkRemoteControlAgent\(\{ quiet: true \}\)/)
  assert.doesNotMatch(roomVue, /forwardRemoteControlCommandToAgent/)
  assert.doesNotMatch(roomVue, /checkRemoteControlAgent/)
})
...
test('共享区底部不再重复显示标题，且图片屏幕游戏菜单都补齐全屏按钮', () => {
  assert.match(roomVue, /<div class="game-panel-tools">[\s\S]*class="ghost-btn stage-fullscreen-btn"[\s\S]*@click\.stop="toggleSharedStageFullscreen"/)
  assert.match(roomVue, /<div class="game-panel-tools">[\s\S]*class="ghost-btn danger close-stage-btn"[\s\S]*@click\.stop="closeGameStage"/)
  assert.match(roomVue, /<div v-if="activeShare\.kind !== 'webpage'" class="share-footer">[\s\S]*<div class="share-meta">[\s\S]*<div v-if="activeShare\.kind === 'video'" class="video-control-panel">/s)
  assert.doesNotMatch(roomVue, /<div class="share-name">\{\{ activeShare\.fileName \}\}<\/div>/)
  assert.doesNotMatch(roomVue, /<div class="share-subtitle">[\s\S]*shareStatusText[\s\S]*<\/div>/)
  assert.match(roomVue, /<div class="share-actions">[\s\S]*v-if="activeShare\.kind === 'image' && canManageSharedMedia" class="ghost-btn" @click="toggleSharedImageZoom"[\s\S]*v-if="activeShare\.kind !== 'video'" class="ghost-btn stage-fullscreen-btn" @click="toggleSharedStageFullscreen">/s)
  assert.match(roomVue, /function toggleSharedStageFullscreen\(\) \{[\s\S]*const stage = sharedStageRef\.value[\s\S]*stage\.requestFullscreen\(\)\.catch/)
  assert.match(roomVue, /function closeGameStage\(\) \{[\s\S]*socket\.value\?\.emit\('game-stage-close'/)
  assert.match(roomVue, /function toggleSharedVideoFullscreen\(\) \{[\s\S]*toggleSharedStageFullscreen\(\)/)
  assert.match(roomVue, /\.stage-fullscreen-btn\s*\{[\s\S]*display:\s*inline-flex;[\s\S]*align-items:\s*center;[\s\S]*justify-content:\s*center;/s)
  assert.match(roomServer, /socket\.on\('game-stage-close', \(payload = \{\}\) => \{[\s\S]*isAdminSocket\(room, socket\.id\)/)
})
...
test('管理员全局视频静音会进入同步协议，且全屏控制条按容器比例缩放', () => {
  ...
  assert.match(roomVue, /action:\s*'ready'[\s\S]*duration:\s*Number\(media\.duration \|\| 0\),[\s\S]*muted:\s*false/)
...
test('视频文件共享默认优先走实时流共享，不支持时再回退文件模式', () => {
  assert.match(roomVue, /if \(kind === 'video' && supportsStreamVideoShare\(\)\) \{[\s\S]*startRealtimeVideoShare\(file\)[\s\S]*return[\s\S]*\}/)
...
test('超级管理员可撤销普通管理员，普通成员仍仅本地控制', () => {
  ...
  assert.match(roomVue, /revokeAdminFrom\(peer\)/)
...
test('WebRTC 重协商会复用本地与共享 sender，避免 m-line 顺序漂移', () => {
  ...
  assert.match(roomVue, /function isPolitePeer\(peerId\) \{[\s\S]*selfId\.value > peerId/)
  assert.match(roomVue, /if \(offerCollision && isPolitePeer\(senderId\)\) \{[\s\S]*await pc\.setLocalDescription\(\{ type: 'rollback' \}\)/)
})
...
test('数据通道异常关闭后会补发共享同步，并忽略预期的 Close called 噪声错误', () => {
  assert.match(roomVue, /function hasIncompleteIncomingTransfer\(peerId, mediaId = activeShare\.value\?\.id\) \{[\s\S]*incomingTransfers\[transferKey\(peerId, mediaId\)\]/)
...
test('多人房间会自动降低 WebRTC 视频负载，并在共享流卡住时自动请求重同步', () => {
  assert.match(roomVue, /function getMeshVideoQualityProfile\(peerCount = otherParticipants\.value\.length\) \{/)
  assert.match(roomVue, /function applySenderVideoQualityProfile\(sender, profile = getMeshVideoQualityProfile\(\)\) \{/)
  assert.match(roomVue, /function applyScreenShareTrackConstraints\(track, profile = getMeshVideoQualityProfile\(\)\) \{/)
  assert.match(roomVue, /function refreshOutgoingMediaQuality\(\) \{[\s\S]*applyLocalStreamQualityProfile\(\)[\s\S]*applySharedStreamQualityProfile\(\)/)
...
test('共享舞台实时流不会再次回退到成员视频宫格，避免成员看到两份相同画面', () => {
  assert.match(roomVue, /function pickPrimaryPeerStream\(peerId\) \{[\s\S]*activeShare\.value\.ownerId === peerId/)
})

test('管理员控制实时视频流时，本地与服务端都会以共享者作为持续同步源', () => {
  assert.match(roomVue, /function emitShareControl\(action, extra = \{\}\) \{[\s\S]*const controllerId = isStreamShare\(activeShare\.value\) && activeShare\.value\.ownerId !== selfId\.value[\s\S]*\?\s*activeShare\.value\.ownerId[\s\S]*:\s*selfId\.value/)
  assert.match(roomServer, /controllerId:\s*room\.sharedMedia\.deliveryMode === 'stream' && room\.sharedMedia\.ownerId[\s\S]*\?\s*room\.sharedMedia\.ownerId[\s\S]*:\s*socket\.id/)
  assert.match(roomServer, /io\.to\(payload\.roomId\)\.emit\('share-control'/)
})

test('自动播放被拦截后的静音重试不应永久锁定成员的本地静音 override，管理员后续仍可下发全局声音状态', () => {
  assert.match(roomVue, /if \(error\?\.name === 'NotAllowedError' && !video\.muted\) \{[\s\S]*video\.muted = true[\s\S]*sharedVideoMuted\.value = true[\s\S]*sharedVideoUi\.muted = true/)
  assert.doesNotMatch(roomVue, /if \(error\?\.name === 'NotAllowedError' && !video\.muted\) \{[\s\S]*sharedVideoHasLocalMuteOverride\.value = true/)
})

test('实时视频共享会在 loadedmetadata\\/canplay 阶段预建共享流，而不是等到播放后才建流，避免短视频成员端看不到画面', () => {
  assert.match(roomVue, /function handleSharedVideoLoaded\(\) \{[\s\S]*if \([\s\S]*activeShare\.value\?\.kind === 'video'[\s\S]*activeShare\.value\.ownerId === selfId\.value[\s\S]*isStreamShare\(activeShare\.value\)[\s\S]*!sharedOutgoingStream\.value[\s\S]*\) \{[\s\S]*initializeOwnedSharedVideoStream\(\)/)
  assert.match(roomVue, /function handleSharedVideoCanPlay\(\) \{[\s\S]*if \([\s\S]*activeShare\.value\?\.kind === 'video'[\s\S]*activeShare\.value\.ownerId === selfId\.value[\s\S]*isStreamShare\(activeShare\.value\)[\s\S]*!sharedOutgoingStream\.value[\s\S]*\) \{[\s\S]*initializeOwnedSharedVideoStream\(\)/)
})
...
test('实时流视频在共享区域内按比例适配，长宽均不超过容器', () => {
  ...
  assert.match(roomVue, /\.shared-video\s*\{[^}]*width:\s*100%/)
  assert.match(roomVue, /\.shared-video\s*\{[^}]*height:\s*100%/)
  assert.match(roomVue, /\.shared-video\s*\{[^}]*object-fit:\s*contain/)
  assert.match(roomVue, /share-content-frame\s*\{[^}]*width:\s*100%/)
  assert.match(roomVue, /share-content-frame\s*\{[^}]*height:\s*100%/)
  assert.match(roomVue, /shared-stage\s*\{[^}]*overflow:\s*hidden/)
})
...
test('管理员声音按钮是全局同步的，普通成员声音按钮只控制本地静音状态', () => {
  ...
  assert.match(roomVue, /function toggleSharedVideoMute\(\) \{[\s\S]*sharedVideoMuted\.value = !sharedVideoMuted\.value/)
})
