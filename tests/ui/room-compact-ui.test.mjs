...
test('共享视频同步播放会避免重复 play 报 AbortError，并在自动播放被拦截时回退为静音重试', () => {
  ...
  assert.match(roomVue, /async function playSharedVideoSafely\(options = \{\}\) \{[\s\S]*if \(!video\.paused && !video\.ended\) \{[\s\S]*return true/)
  ...
  assert.match(roomVue, /if \(sync\.playing\) \{[\s\S]*if \(video\.paused \|\| video\.ended\) \{[\s\S]*playSharedVideoSafely\(\{ source: '同步播放' \}\)/)
})
