...
test('屏幕共享启动不再自动探测本地远控 agent，而是收到真实远控指令后再按需检查', () => {
  assert.doesNotMatch(roomVue, /async function startScreenShare\(\) \{[\s\S]*checkRemoteControlAgent\(\{ quiet: true \}\)/)
  assert.match(roomVue, /async function forwardRemoteControlCommandToAgent\(command, senderName = ''\) \{[\s\S]*const ready = await checkRemoteControlAgent\(\{ quiet: true \}\)/)
})
...
test('共享舞台实时流不会再次回退到成员视频宫格，避免成员看到两份相同画面', () => {
  assert.match(roomVue, /function pickPrimaryPeerStream\(peerId\) \{[\s\S]*activeShare\.value\.ownerId === peerId[\s\S]*stream\.id !== \(sharedIncomingStream\.value\?\.id \|\| activeShare\.value\.streamId\)[\s\S]*\|\| null/)
})
