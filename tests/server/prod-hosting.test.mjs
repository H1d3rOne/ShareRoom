...
test('install.sh 与部署文档改为单端口 3002 反代生产服务', () => {
  ...
  assert.match(installScript, /proxy_pass http:\/\/127\.0\.0\.1:\$app_port|proxy_pass http:\/\/127\.0\.0\.1:\$APP_PORT|proxy_pass http:\/\/127\.0\.0\.1:\$\{APP_PORT\}/)
  assert.match(httpsGuide, /npm run serve/)
  assert.match(httpsGuide, /proxy_pass http:\/\/127\.0\.0\.1:3002/)
  assert.doesNotMatch(httpsGuide, /proxy_pass http:\/\/127\.0\.0\.1:3001/)
})
