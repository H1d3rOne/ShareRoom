...
test('install.sh 支持交互式域名、HTTP/HTTPS 与 Nginx webroot 证书配置', () => {
  ...
  assert.match(installScript, /8443 已由 Nginx 监听，将继续复用 8443/)
  ...
  assert.match(installScript, /for attempt in \$\(seq 1 10\)/)
  assert.match(installScript, /sleep 0\.2/)
  assert.match(installScript, /verify_https_listener\(\)/)
  assert.match(installScript, /port_in_use\(\)/)
  assert.match(installScript, /ss -H -tln/)
  assert.match(installScript, /sport = :\$\{port\}/)
  ...
  assert.match(installScript, /nginx_version_supports_modern_http2\(\)/)
  assert.match(installScript, /listen \$HTTPS_PORT ssl;/)
  assert.match(installScript, /http2 on;/)
  assert.match(installScript, /listen \$HTTPS_PORT ssl http2;/)
...
test('HTTPS 部署文档说明 install.sh 使用 certbot webroot 配置 Nginx', () => {
  ...
  assert.match(httpsGuide, /443.*8443|8443.*443/)
  ...
  assert.match(httpsGuide, /http2 on;|listen 443 ssl http2;/)
